;;; comint-logo.el --- general command interpreter in a window stuff
...
;;
;; 2d-nonsequential additions
;;  comint-terminal-return              function ...
;;  comint-strip-input-function         function ...
;;  comint-insert-previous-input-function function ...
;;
...
;; 2d-nonsequential additions
(defvar comint-terminal-return nil
  "Function that inserts hard or soft newline.")

(defvar comint-strip-input-function nil
  "Use this function to modify input sent to prcess. Used to remove newlines
in logo-mode.")

(defvar comint-insert-previous-input-function nil
  "Function that inserts matching input.")

(defvar process-defines-term nil "Pacifying the compiler.")
...
(put 'comint-insert-previous-input-function'permanent-local t)
...
Entry to this mode runs the hooks on `comint-mode-hook'."
  ...
  ;; 2d-nonsequential additions
  (make-local-variable 'comint-terminal-return)
  (setq comint-terminal-return (symbol-function 'identity))
  (make-local-variable 'comint-strip-input-function)
  (setq comint-strip-input-function (symbol-function 'identity))
  (make-local-variable 'comint-insert-previous-input-function)
  (setq comint-insert-previous-input-function
	(function comint-insert-previous-matching-input))
  ;; 
...
(defun comint-exec-1 (name buffer command switches)
  (let ((process-environment
	 (nconc
	  ...
	  ;;
	  ;; Some inferior process modes can define terminal type other
	  ;; than `emacs' or `dumb' -- for an example see `logo.el'.
	  ;; To use this, bind temporarily process-defines-term to a
	  ;; list of three elements, as in following example
	  ;;
	  ;;  (let ((process-defines-term
	  ;;         (list logo-term logo-term-columns logo-term-rows)))
          ;;    (set-buffer
	  ;;        (apply 'make-comint "logo" logo-binary-name nil nil)))
	  ;;
	  (cond
	   ((and (boundp 'process-defines-term) process-defines-term)
	    (list (concat "TERM=" (car process-defines-term))
		  (format "COLUMNS=%d" (cadr process-defines-term))
		  (format "LINES=%d" (car (cddr process-defines-term)))))
...
If N is negative, find the next or Nth next match."
  ...
  (let ((pos (comint-previous-matching-input-string-position regexp arg)))
	...
	(error "Not found")
      ...
      (funcall comint-insert-previous-input-function
	       (ring-ref comint-input-ring pos)))))

(defun comint-insert-previous-matching-input (input)
  (delete-region 
  ...
  (let ((proc (get-buffer-process (current-buffer))))
	...
	(let* ((pmark (process-mark proc))
	       (intxt (if (>= (point) (marker-position pmark))
			  (progn
			    (cond ((eq comint-eol-on-send 'end-of-input)
				   (goto-char comint-last-input-end))
				  (comint-eol-on-send (end-of-line)))
			    (buffer-substring pmark (point)))
			(let ((copy (funcall comint-get-old-input)))
			  (goto-char pmark)
			  (insert copy)
			  copy)))
	       (input (funcall comint-strip-input-function
			       (if (not (eq comint-input-autoexpand 'input))
	    ...
	    (insert-before-markers
		     (funcall comint-terminal-return ?\n)))
