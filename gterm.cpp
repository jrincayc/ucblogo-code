...
void GTerm::PassInputToGterm(int len, unsigned char *data)
{
  // data[len] = '\0';
  //printf("%s\n", (char *)data);
...
void GTerm::ResizeTerminal(int w, int h)
{
	...
	if(cursor_y > scroll_bot)
		scroll_region(scroll_top, cursor_y, cursor_y - scroll_bot);
	cx = min(width-1, cursor_x);
	cy = min(height-1, cursor_y);
	clear_area(0, min(oldheight,h), min(oldwidth,w)-1, MAXHEIGHT-1);
	move_cursor(cx, cy);
}
