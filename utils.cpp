...
/* Creates an HTML wxString representing the text in the terminal.
   Note: you must delete this string when you are done with it.
*/
wxString* GTerm::get_text(){
	wxString *textString = new wxString();
	textString->clear();
	int i;
    ...
    for (y=0; y<height; y++) {
		...
		for (x=start_x; x<=width; x++) {
			...
			if (c != color[yp+x]) {
				if (!blank) {
					if(mode_flags & PC)
						DrawText(0,15, c/*&15*/, start_x,
								 y, x-start_x, text+yp+start_x);
					else
						DrawText(0, 15, c/*&15*/, start_x,
								 y, x-start_x, text+yp+start_x);
				} else {
					ClearChars(15, start_x, y, x-start_x, 1);
				}
				start_x = x;
				c = color[yp+x];
				blank = !(mode_flags & TEXTONLY);
...
void GTerm::make_line_dirty(int line){
		dirty_startx[line] = 0;
	    update_changes();
}
...
{
	// to get rid of the old cursor
	dirty_startx[end_y] = 0;  
	dirty_endx[end_y] = MAXWIDTH-1;
	...
	if (!num) return;
	mx = end_y-start_y+1;
	wxdprintf("num is: %d, mx is: %d", num, mx);
	if (num > mx) num = mx;
	if (-num > mx) num = -mx;
...
void GTerm::copy_text(int y_init, int y_final, int xpos_init, int xpos_final, int num) {
  int yp_init = linenumbers[y_init]*MAXWIDTH;
  int yp_final = linenumbers[y_final]*MAXWIDTH;
  memmove(text+yp_final + xpos_final, text+yp_init + xpos_init, num);
  memmove(color+yp_final + xpos_final, color+yp_init  + xpos_init, num);
}

void GTerm::shift_text(int y, int start_x, int end_x, int num)
{
     int x, yp, mx, c, ypnext;
