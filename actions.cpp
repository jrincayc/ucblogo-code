...
char str[100];
	...
	if (*input_data < 32 && *input_data != 17) return;
	...
	n = 0;
	if (mode_flags & NOEOLWRAP) {
		while ((input_data[n]>31 || input_data[n] == 17) && n<data_len) n++;
		n_taken = n;
		if (cursor_x+n>=width) n = width-cursor_x;
	} else {
		while ((input_data[n]>31 || input_data[n] == 17) && n<data_len && cursor_x+n<width) n++;
		n_taken = n;
	}
	...
	for (i=0; i<n; i++) {
	  if (input_data[i] == (unsigned char)17) {
	    invert_on = !invert_on;
	    if (invert_on)
...
extern int last_logo_y;
extern int cur_y;
void GTerm::lf()
{
  //****************************** Added by Alan******************
  if ( mode_flags & INSERT ) {
      scroll_region(cursor_y+1, scroll_bot, -1);
      update_changes();
      shift_text(cursor_y+1, 0, width-1, width - 1 - cursor_x);
      copy_text(cursor_y, cursor_y + 1, cursor_x, 0, width - 1 - cursor_x);
