...
/* USUAL.FOO has to be recognized in paren.c as a reference to the FOO method
   in the parent(s), provided we are in the body of FOO, so that we know how many
   inputs it takes.  Then in eval.c we have to look for the parent(s)' FOO method
   So there is no lusual() -- it's not a Logo primitive */
