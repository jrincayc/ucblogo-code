...
void TurtleCanvas::realDrawLabel(char *data, wxDC *dc) {
    ...
    dc->SetBackgroundMode(wxSOLID);
    dc->SetTextBackground(colorname[turtleFrame->back_ground]);
    dc->SetTextForeground(colorname[turtleFrame->xgr_pen.color]);
    dc->DrawText(s, getPen()->xpos, getPen()->ypos-ht);
    if (!drawToPrinter) {
	m_memDC->SetBackgroundMode(wxSOLID);
	m_memDC->SetTextBackground(colorname[turtleFrame->back_ground]);
	m_memDC->SetTextForeground(colorname[turtleFrame->xgr_pen.color]);
